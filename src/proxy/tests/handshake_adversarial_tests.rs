...
async fn auth_probe_throttle_saturation_stress() {
    ...
    for _ in 0..AUTH_PROBE_BACKOFF_START_FAILS {
        auth_probe_record_failure(proxy_shared.as_ref(), target_ip, now);
    }

    assert!(auth_probe_is_throttled(proxy_shared.as_ref(), target_ip, now));

    // Stress test with many unique IPs
    for i in 0..500u32 {
        let ip = IpAddr::V4(Ipv4Addr::new(203, 0, 113, (i % 256) as u8));
        auth_probe_record_failure(proxy_shared.as_ref(), ip, now);
    }

    let tracked = proxy_shared.auth_probe.len();
    assert!(
...
async fn auth_probe_success_clears_throttled_peer_state() {
    ...
    let now = Instant::now();
    for _ in 0..AUTH_PROBE_BACKOFF_START_FAILS {
        auth_probe_record_failure(shared.as_ref(), target_ip, now);
    }
    assert!(auth_probe_is_throttled(shared.as_ref(), target_ip, now));

    auth_probe_record_success(shared.as_ref(), target_ip);
    assert!(
        !auth_probe_is_throttled(shared.as_ref(), target_ip, now + Duration::from_millis(1)),
