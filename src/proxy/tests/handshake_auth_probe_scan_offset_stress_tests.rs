...
fn positive_same_ip_moving_time_yields_diverse_scan_offsets() {
    ...
    for i in 0..512u64 {
        ...
        let offset =
            super::auth_probe_scan_start_offset(shared.as_ref(), ip, now, 65_536, 16);
        uniq.insert(offset);
    }
...
fn adversarial_many_ips_same_time_spreads_offsets_without_bias_collapse() {
    ...
    for i in 0..1024u32 {
        ...
        uniq.insert(super::auth_probe_scan_start_offset(
            shared.as_ref(),
            ip,
            now,
            65_536,
...
async fn stress_parallel_failure_churn_under_saturation_remains_capped_and_live() {
    ...
    assert!(
        shared.auth_probe.len() <= AUTH_PROBE_TRACK_MAX_ENTRIES,
        "state must remain hard-capped under parallel saturation churn"
    );

    let probe = IpAddr::V4(Ipv4Addr::new(10, 4, 1, 1));
    let _ = super::auth_probe_should_apply_preauth_throttle(
        shared.as_ref(),
        probe,
...
fn light_fuzz_scan_offset_stays_within_window_for_randomized_inputs() {
    ...
    for _ in 0..8192 {
        ...
        let offset =
            super::auth_probe_scan_start_offset(shared.as_ref(), ip, now, state_len, scan_limit);
        assert!(
            offset < state_len,
